  ...
  namespace dreamview
  {
    ...
    void SimControl::InitTimerAndIO()
    {
      ...
      sim_control_timer_.reset(new cyber::Timer(
          kSimControlIntervalMs, [this]()
          { this->RunOnce(); },
          false));
      sim_prediction_timer_.reset(new cyber::Timer(
          kSimPredictionIntervalMs, [this]()
          { this->PublishDummyPrediction(); },
          false));
    }
      ...
      else
      {
        ...
        else
        {
          ...
          while (last_localization_index < trajectory.size() - 1)
          {
            ...
            auto d1 = std::hypot(x1 - next_point_.path_point().x(),
                                 y1 - next_point_.path_point().y());
            auto d2 = std::hypot(x2 - next_point_.path_point().x(),
                                 y2 - next_point_.path_point().y());
            if (d1 < d2)
            {
              break;
            }
          ...
          auto planned_time =
              trajectory.Get(last_localization_index).relative_time() +
